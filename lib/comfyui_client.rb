...
class ComfyuiClient < ImageGenerationClient
  ...
  def initialize(comfyui_url, token = nil, workflow_path = "workflows")
    @http_client = ComfyuiHttpClient.new(comfyui_url, token)
  ...
  def generate_and_wait(params, max_wait_seconds = 1000, &block)
    ...
    result = http_client.queue_prompt(workflow)
    ...
    loop do
      ...
      queue = http_client.get_prompt_queue
      ...
      status = http_client.get_prompt_status(prompt_id)
      ...
      if status&.dig(prompt_id, "status", "status_str") == "error"
        ...
        if (outputs = status.dig(prompt_id, "outputs", params[:output], "images"))
          ...
          image_data = http_client.get_image(image_info["filename"], image_info["subfolder"], image_info["type"])
          ws&.close
          return {
